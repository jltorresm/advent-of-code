...
function get_score($input) {
	...
	foreach (explode("\n", $input) as $game) {
		...
		if (in_array($game, $winners)) {
			// Win
			$total_score += $choice_score[$game[1]] + $win_score;
		} else if (in_array($game, $draws)) {
			// Draw
			$total_score += $choice_score[$game[1]] + $draw_score;
		} else {
			// Loose
			$total_score += $choice_score[$game[1]] + $loose_score;
		}
	}
